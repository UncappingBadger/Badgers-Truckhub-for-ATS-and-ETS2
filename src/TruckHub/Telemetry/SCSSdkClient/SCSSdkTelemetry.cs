...
namespace SCSSdkClient {
    ...
    public class SCSSdkTelemetry: IDisposable {
        ...
        private void _updateTimer_Elapsed(object sender) {
            ...
            // Seed all the special-event tracking flags from the first real snapshot instead of
            // their false default, so leftover "true" bits already sitting in shared memory from
            // earlier in the game session (job already active, a fine/tollgate/refuel that happened
            // before this process connected) don't get misread as events happening right now.
            if (!initialized) {
                initialized = true;
